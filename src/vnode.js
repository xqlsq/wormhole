...
function createElm(vnode) {
    if (vnode.tag === undefined) {
        ...
        if (typeof vnode.tag === 'function') { // 创建新dom 判断如果是组件，自带一个更新功能
...
class Dom {
    ...
    updateChildren(parentElement, oldVnodes, vnodes) {
        ...
        vnodes.forEach((vnode, index) => {
            ...
            if (oldIdents[key]) {
                ...
                index ? 
                    this.insertAfterVnode(this.createElm(vnode), vnodes[index - 1].elm) :
                    this.insertBeforeVnode(this.createElm(vnode), oldVnodes[0].elm);
            }
        });
    ...
    patchVnode(oldVnode, vnode) {
        ...
        if (oldVnode.text === undefined && vnode.text === undefined) {
            if (!isEmptyChildren(oldChildren) && !isEmptyChildren(newChildren)) this.updateChildren(elm, oldChildren, newChildren);
            else if (!isEmptyChildren(oldChildren)) elm.textContent = ''; // 清空
            else if (!isEmptyChildren(newChildren)) newChildren.forEach((_vnode) => elm.appendChild(this.createElm(_vnode)));
            else return;
        } else {
...
Dom.vnode = function(tag, props, ...children) {
    if (typeof tag === 'function') { // 如果是自定义组件
    ...
    children && (_children = []) && children.forEach((c) => {
        if (Array.isArray(c)) _children = _children.concat(c)
        else _children.push(c)
    });
    if (props) {
